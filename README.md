...
Zone magiche per setup multi-monitor su Windows 10/11. Trascini una finestra: sopra tutti i monitor
appaiono le zone. La rilasci su una zona e la finestra ci si incastra. Se invece la **lanci**
(rilascio mentre il mouse è ancora in movimento veloce), vola nella zona che si trova in quella
direzione, anche su un altro monitor.
...
|---|---|---|
| `activation` | `"always"` | `"always"` = zone a ogni trascinamento, `"shift"` = solo tenendo Shift |
| `throwEnabled` | `true` | Abilita il lancio |
...
- `bin\MagicZones.exe --preview .\preview` salva in PNG overlay ed editor di ogni monitor senza mostrarli.
